module.exports = function (RED) {
    ...
    function handleErrors(error, node, msg) {
        ...
        // If PresetQuery Value Invalid
            ...
            var inDepthError = 'PresetQuery Value Invalid:\n' +
            ...
            try {
                ...
                if (historyQuery) {
                    ...
                    if (historyQuery.start) {
                        ...
                        else { throw "HistoryQuery Start Invalid Timestamp"; }
                    }
                    if (historyQuery.end) {
                        var end = new Date(historyQuery.end);
                        if (end.getTime()) { historyQuery.end = end.toISOString(); }
                        else { throw "HistoryQuery End Invalid Timestamp"; }
                    }
                    if (historyQuery.limit) {
                        if (Number.isInteger(Number(historyQuery.limit))) { null }
                        else { throw "HistoryQuery Limit Invalid"; }
                    }
